...
import { createUserAndAgency } from "@/lib/firestore/agency";
...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await updateProfile(credential.user, { displayName: `${name} · ${agency}` });
      await createUserAndAgency(credential.user.uid, name, credential.user.email!, agency);
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-secondary/30 px-4">
      ...
      <Card className="w-full max-w-sm border border-border shadow-none">
        <CardHeader className="text-center pb-4">
          ...
          <p className="text-base text-muted-foreground">Start your 14-day free trial</p>
        </CardHeader>
        <CardContent>
          <Button
            variant="outline"
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="agency" className="text-sm">Agency name</Label>
              <Input
                id="agency"
                type="text"
